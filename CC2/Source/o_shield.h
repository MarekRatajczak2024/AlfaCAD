...
#ifndef ALFACAD_O_SHIELD_H
#define ALFACAD_O_SHIELD_H

extern void Shield_analysis(void);
...
typedef struct
{
    ...
    int body;  //number of 0 plate 2 wall or 3 zone
    int body_no;  //number of plate or zone or wall
    int edge_no;  //number of edge of plate or zone or wall
    int emb_no;
} SH_NODE_EMB_EDGE;
...
typedef struct
{
    ...
    int point_edge_no;  //number of edge on plate or zone
    ...
    int factor_record;
    int take_it;
    int style;  //compativle with vector.style
    int type;   //0 concentrated force load in xy plane Fx, Fy, 1 uniformly distributed load in xy plane downward, 2 linear evenly (uniformly) or trapezoidally distributed load on edge in xy plane fx, fy
    int k; //the edge where load is applied for distributed load
    float x1;
    float y1;
    float x2;
    float y2;
    double magnitude1x;  //for style==4 in shields means fx
    double magnitude2x;  //for style==4 in shields means fx
    double magnitude1y;  //for style==4 in shields means fy
    double magnitude2y;  //for style==4 in shields means fy
} SH_LOAD;
