...
static TMENU mDialogCursor = { 3,0,0,8,23,9,ICONS,CMNU,CMBR,CMTX,0,27,0,0,0,(POLE(*)[]) &pmDialogCursor,NULL,NULL }; //22
...
static POLE pmOpacity[] = {
   {u8"Заливки",L'З',866,&mTranslucency},
   {u8"Градієнти",L'Г',867,&mGTranslucency},
};
...
#define chprop_tips18 u8"Непрозорість заповнення"
#define chprop_tips19 u8"Стиль вектора для координат"
...
char *vector_txt[]={u8"Вектор: жорсткий-жорсткий",u8"Вектор: жорсткий-штифт",u8"Вектор: pin-жорсткий",u8"Вектор: pin-pin",u8"Вектор: Сила",u8"Вектор: Момент" ,u8"Вектор: -Момент",
 ...
 u8"Вектор: Сила Z",u8"Вектор: навантаження трапеції Z", u8"Вектор: Момент X",u8"Вектор: -Момент X",u8"Вектор: Момент Y",u8"Вектор: -Момент Y",u8"Вектор: Момент XY",u8"Вектор: -Момент XY",
 u8"Вектор: переміщення Z", u8"Вектор: обертання X",u8"Вектор: -обертання X",u8"Вектор: обертання Y",u8"Вектор: -обертання Y",u8"Вектор: обертання XY",u8"Вектор: -обертання XY",
 };
...
static POLE pmPLine[] = {
        ...
        {u8"Без опори\0", L'Б', 851,NULL},
        {u8"Шарнірно-нерухома опора\0", L'Ш', 850,NULL},
        {u8"Жорстке затиснення\0", L'Ж', 852,NULL},
        {u8"шарнірно-Рухома опора\0", L'Р', 849,NULL},
        {u8"Перевернути опору\0", L'П', 853,NULL},
};
...
POLE pmEdgeType[] = {
        ...
        {u8"Шарнірно-нерухома опора\0", L'Ш', 850,NULL},
        {u8"Жорстке затиснення\0", L'Ж', 852,NULL},
        {u8"Рухома опора\0", L'Р', 849,NULL},
        };
...
char *Vector_txt[]={u8"жорсткий-жорсткий",
                    u8"жорсткий-шарнірний",
                    u8"шарнірний-жорсткий",
                    u8"шарнірний-жорсткий",
                    u8"Сила",
                    u8"Момент+",
                    u8"Момент-",
                    ...
                    u8"поверхневе навантаження плити",
                    u8"зосереджене навантаження плити",
                    u8"Сила Z",
                    u8"трапеція Z",
                    u8"Момент X",
                    u8"-момент X",
                    ...
                    u8"Обертання XY",
                    u8"-обертання XY",
                    u8"балка на фундаменті Вінклера"};
...
static POLE pmVector_cartesian[] = {
          ...
          {u8"-обертання\0\0",L'Е',732,NULL}, //&mLoad_Char}, //NULL
          ...
          ////for grids
          ////{u8"Сила Z\0\0",L'1',880,&mLoad_Char},
          ////{u8"трапеція Z\0\0",L'2',881,&mLoad_Char},
          ////{u8"Момент X\0\0",L'3',882,&mLoad_Char},
          ////{u8"-момент X\0\0",L'4',883,&mLoad_Char},
          ////{u8"Момент Y\0\0",L'5',884,&mLoad_Char},
          ////{u8"-момент Y\0\0",L'6',885,&mLoad_Char},
          ////{u8"Момент XY\0\0",L'7',886,&mLoad_Char},
          ////{u8"-момент XY\0\0",L'8',887,&mLoad_Char},

          ////{u8"Переміщення Z\0\0",L'9',888,NULL},
          ////{u8"Обертання XZ\0\0",L'!',889,NULL},
          ////{u8"-обертання XZ\0\0",L'@',890,NULL},
          ////{u8"Обертання YZ\0\0",L'#',891,NULL},
          ////{u8"-обертання YZ\0\0",L'$',892,NULL},
          ////{u8"Обертання XY\0\0",L'%',893,NULL},
          ////{u8"-обертання XY\0\0",L'^',894,NULL},
};

static POLE pmVector_isometric[] = {
		  {u8"Зняти\0Del\0",L'З',218,NULL},
          {u8"жорсткий - жорсткий\0\0",L'А',723,NULL},
          {u8"балка на фундаменті Вінклера\0\0",L'В',878,NULL},
          ////{u8"жорсткий - штифт\0\0",L'Б',724,NULL},
          ////{u8"штифт - жорсткий\0\0",L'В',725,NULL},
          ////{u8"штифт - штифт\0\0",L'Г',726,NULL},
          {u8"Сила\0\0",L'С',727,&mLoad_Char},
...
static POLE pmForce_Displacement_style_isometric[] = {
          {u8"Сила Z\0\0",L'С',880,NULL},
          {u8"Переміщення Z\0\0",L'П',888,NULL},
};
...
static POLE pmMoment_Rotation_style_isometric[] = {
          {u8"Момент X\0\0",L'А',882,NULL},
          {u8"-момент X\0\0",L'Б',883,NULL},
          {u8"Момент Y\0\0",L'В',884,NULL},
          {u8"-момент Y\0\0",L'Г',885,NULL},
          {u8"Момент XY\0\0",L'Д',886,NULL},
          {u8"-момент XY\0\0",L'Е',887,NULL},
          {u8"Обертання X\0\0",L'Ж',889,NULL},
          {u8"-бертання X\0\0",L'З',890,NULL},
          {u8"Обертання Y\0\0",L'И',891,NULL},
          {u8"-бертання Y\0\0",L'К',892,NULL},
          {u8"Обертання XY\0\0",L'Л',893,NULL},
          {u8"-бертання XY\0\0",L'М',894,NULL},
};
...
static POLE pmLoad_style[] = {
          ...
          {u8"трапеція Z\0\0",L'З',881,NULL},
};
...
static POLE pmPLine_Con_Slab[] = {
		  {u8"Продовження",L'П',220,NULL},
          {u8"Вектори",L'В',722,NULL},  //mVector will be done later
};
...
#define _CHANGE_POLYLINE_TYPE_ u8"Вибрано інший тип полілінії. Чи варто змінити тип на: "
...
static POLE pmTyp_punktu[] = {
     ...
     {u8"жорстке ковзаюча по осі X\0",L'Х',870,NULL},
     {u8"жорстке ковзаюча по осі X(В)\0",L'А',871,NULL},
     {u8"жорстке ковзаюча по осі Y\0",L'У',872,NULL},
     {u8"жорстке ковзаюча по осі Y(В)\0",L'В',873,NULL},
     {u8"шарнірне ковзаюча по осі X\0",L'Д',874,NULL},
     {u8"шарнірне ковзаюча по осі X(В)\0",L'Е',875,NULL},
     {u8"шарнірне ковзаюча по осі Y\0",L'И',876,NULL},
     {u8"шарнірне ковзаюча по осі Y(В)\0",L'М',877,NULL},
};
...
char* view_edge_tab[] = {u8"Без опори", u8"", u8"", u8"", u8"Роликова опора", u8"Без опори", u8"Шарнірно-нерухома опора", u8"Жорстке затиснення"};
char* view_edgeinverted_tab[] = {u8"Звичайний", u8"Перевернутий"};
...
static POLE pmKierunek[] = {
		...
		{u8"під кутом До",L'Д',206,NULL},
		{u8"під Заданим кутом",L'З',207,NULL},
		{u8"Розміровка кута",L'Р',208,NULL},
        {u8"розмірування Kола/дуги",L'K',202,NULL},
